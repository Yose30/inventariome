<template>
    <div>
        <div v-if="mostrar && !detalles && !editar">
            <div class="row">
        ...
        <div v-if="editar">
            ...
            <hr>
            <div v-if="datosRemision">
                <b-card>
                    <template v-slot:header>
                        <b-row>
                            <b-col sm="10"><h6><b>Datos del cliente</b></h6></b-col>
                            <b-col sm="2">
                                <b-button 
                                    variant="warning" 
                                    @click="editarInformacion()">Editar
                                </b-button>
                            </b-col>
                        </b-row>
                    </template>
                    <b-card-text>
                        <b>Nombre:</b> {{ dato.name }}<br>
                        <b>Dirección:</b> {{ dato.direccion  }}<br>
                        <b>Correo electrónico:</b> {{ dato.email }}<br>
                    </b-card-text>
                </b-card>
                <hr>
                <b-table :items="registros" :fields="fieldsDatos" responsive>
                    ...
                    <template slot="eliminar" slot-scope="row">
                        <b-button v-if="remision.estado == 'Iniciado'" @click="eliminarDato(row.item, row.index)" variant="danger"><i class="fa fa-minus-circle"></i></b-button>
                    </template>
                    <template slot="thead-top" slot-scope="row">
                        <tr>
                            <th>ISBN</th>
                    ...
                    <b-row>
                        ...
                        <b-col>
                            <b-row>
                                ...
                                <b-col sm="10">
                                    <b-input v-model="queryCliente" autofocus @keyup="coincidenciaCliente()"></b-input>
                                </b-col>
                            </b-row>
                        </b-col>
                    </b-row>
        ...
        <!-- ELIMINAR DATO DE REMISIÓN -->
        <b-modal ref="modal-eliminarD" title="Eliminar registro" hide-footer>
            <p class="my-4">¿Seguro de eliminar el registro?</p>
            <div class="text-right">
                <b-button @click="confirmarEliminar()" variant="danger">Confirmar</b-button>
            </div>
        </b-modal>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                depositos: [],
                // NUEVAS VARIABLES PARA EDITAR LA REMISIÓN
                editar: false,
                dato: {},
                clientes: [],
                datosRemision: false,
        ...
        methods: {
            ...
            editarRemision (remision) {
                axios.get('/lista_datos', {params: {numero: remision.id}}).then(response => {
                    ...
                    this.remision = {};
                    this.remision = remision;
                    this.dato = this.remision.cliente;
                    this.registros = response.data.remision.datos;
                    this.editar = true;
                    this.datosRemision = true;
            ...
            // CAMBIAR EL CLIENTE DE LA REMISIÓN
            editarInformacion () {
                axios.get('/getTodo').then(response => {
                    this.clientes = response.data;
                    this.datosRemision = false;
                }).catch(error => {
                   this.makeToast('danger', 'Ocurrio un problema, vuelve a intentar o actualiza la pagina');
                });
            },
            // MOSTRAR COINCIDENCIAS DEL CLIENTE
            coincidenciaCliente () {
            ...
            // INICIALIZAR DATOS PARA ELIMINAR REGISTRO
            eliminarDato (registro, index) {
                this.datoEliminar = registro;
                this.position = index;
                this.$refs['modal-eliminarD'].show();
            },
            // CONFIRMAR PARA ELIMINAR REGISTRO
            confirmarEliminar () {
                this.eliminados.push(this.datoEliminar);
                this.registros.splice(this.position, 1);
                this.$refs['modal-eliminarD'].hide();
            },
